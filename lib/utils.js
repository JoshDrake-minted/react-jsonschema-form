...
function computeDefaults(schema, parentDefaults) {
  ...
  if (isObject(defaults) && isObject(schema.default)) {
    ...
    // Use referenced schema defaults for this node.
    var allDefaults = schema.allOf.map(function (childSchema) {
      return computeDefaults(childSchema, defaults, definitions);
    });
    return Object.assign.apply(Object, [{}].concat(_toConsumableArray(allDefaults)));
...
function orderProperties(properties, order) {
  ...
  var actualOrder = order.filter(function (prop) {
    return prop === "*" || extraneous.indexOf(prop) === -1;
  });
  var orderHash = arrayToHash(actualOrder);

  var rest = properties.filter(function (prop) {
    return !orderHash[prop];
  });
  var restIndex = actualOrder.indexOf("*");
  if (restIndex === -1) {
    if (rest.length) {
  ...
  var complete = [].concat(_toConsumableArray(actualOrder));
...
function retrieveSchema(schema) {
  ...
  if (schema.hasOwnProperty("allOf")) {
    var _schema = schema.allOf.map(function (childSchema) {
      return retrieveSchema(childSchema, definitions);
    });
    return {
      "type": "object",
      "properties": Object.assign.apply(Object, [{}].concat(_toConsumableArray(_schema.map(function (childSchema) {
        return childSchema.properties;
      })))),
      "title": schema.title
    };
  }
...
function toIdSchema(schema, id, definitions) {
  ...
  if ("allOf" in schema) {
    var _schema2 = schema.allOf.map(function (childSchema) {
      return retrieveSchema(childSchema, definitions);
    });
    return Object.assign.apply(Object, [{}].concat(_toConsumableArray(_schema2.map(function (childSchema) {
      return childSchema.properties;
    }))));
  }
  if ("items" in schema) {
