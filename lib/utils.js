...
function toIdSchema(schema, id, definitions) {
  ...
  if ("allOf" in schema) {
    ...
    return Object.assign.apply(Object, [{}].concat(_toConsumableArray(_schema.map(function (childSchema) {
      return childSchema.properties;
    }))));
  }
  if ("$ref" in schema) {
    var _schema2 = retrieveSchema(schema, definitions);
    return toIdSchema(_schema2, id, definitions);
  }
  if ("items" in schema) {
